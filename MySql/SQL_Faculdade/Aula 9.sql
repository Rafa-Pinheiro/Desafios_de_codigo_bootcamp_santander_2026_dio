--> ANOTAÇÕES BY -> RAFAELA PINHEIRO <- AULA 8 --

--> COPIAR ARQUIVO (.mdf) E (.ldf) PARA A PASTA 
--	-> C:\Program Files\Microsoft SQL Server\MSSQL15.SQLEXPRESS\MSSQL\DATA --
--> HABILITAR SERVIÇOS SQL Server (SQLEXPRESS)
--> ABRIR PROGRAMA Microsoft SQL Server Management Studio
--> CONECTAR
--> BOTÃO DIREITO NO BANCO DE DADOS -> ADICIONAR
--> TABELAS -> BOTÃO DIREITO  -> DESIGN
--> NOVA CONSULTA


USE Universidade;

SELECT * FROM Cursos;
SELECT * FROM Faculdades;

--> AULA PASSADA (6 & 7 & 8) -> COMANDOS <-- 

	/*
	SELECT nm_curso AS 'Curso', vl_curso AS 'Preço', vl_curso*1.1 AS 'Novo Preço' FROM Cursos;                  

	-- O AS da um novo título a coluna

	SELECT MAX(vl_curso) AS 'Maior Valor' FROM Cursos;																		

	-- O MAX demonstra o maior valor

	SELECT MIN(vl_curso) AS 'Menor Valor' FROM Cursos;																		

	-- O MIN demonstra o menor valor

	SELECT AVG(vl_curso) AS 'Média  dos Valores' FROM Cursos;																

	-- O AVG demonstra a média dos valores

	SELECT MAX(vl_curso) AS 'Maior Valor', 
	MIN(vl_curso) AS 'Menor Valor', 
	AVG(vl_curso) AS 'Média  dos Valores' 
	FROM Cursos;		

	-- É possível colocar todos na mesma linha

	SELECT nm_curso, vl_curso FROM Cursos WHERE cd_faculdade = 2;

	SELECT AVG(vl_curso) AS 'Média  dos Valores' 
	FROM Cursos 
	WHERE cd_faculdade = 2;

	SELECT SUM(vl_curso) AS 'Soma dos valores' FROM Cursos;																	

	-- O SUM demonstra a soma dos valores selecionados

	SELECT COUNT(*) AS 'Quantidade de valores' FROM Cursos;																				

	-- O COUNT retorna a quantidade de linhas da tabela

	SELECT COUNT(vl_curso) AS 'Quantidade de valores' FROM Cursos;															

	-- O COUNT retorna a quantidade de linhas da tabela(não nulos) neste caso


	SELECT nm_curso AS 'Curso', 
	vl_curso AS 'Preço', 
	nm_faculdade AS 'Faculdade' 
	FROM Cursos, Faculdades 
	WHERE Cursos.cd_faculdade = Faculdades.cd_faculdade 
	AND vl_curso <> 1500 
	ORDER BY vl_curso ASC;
	-- O WHERE Cursos.cd_faculdade = Faculdades.cd_faculdade interliga as colunas de cursos com as de faculdade(Junção Natural)

	SELECT nm_curso AS 'Curso', 
	vl_curso AS 'Preço', 
	nm_faculdade AS 'Faculdade' 
	FROM Cursos 
	INNER JOIN Faculdades 
	ON Cursos.cd_faculdade = Faculdades.cd_faculdade 
	AND vl_curso <> 0 
	ORDER BY vl_curso ASC; 
	-- O INNER JOIN FAZ A MESMA COISA QUE A FORMA ANTERIOR

	SELECT DISTINCT(vl_curso) FROM Cursos;
	-- DISTINCT SELECIONA APENAS OS DADOS DIFERENTES

	SELECT nm_curso, vl_curso
	FROM Cursos
	WHERE vl_curso BETWEEN 2000 AND 3000;                        
	-- O BETWEEN junto ao AND asão usados para simplificar a escrita de valores na consulta, aqui ele seleciona mensalidades entre 2000 e 3000

	SELECT nm_curso
	FROM Cursos
	WHERE cd_curso IN (1,5,6);

	--O DROP TABLE destrói a tabela inteira(perigoso usar)

	-->** SP - STORED PROCEDURE **<--

	-->INSERIR FACULDADE <--
	CREATE PROC in_Faculdades_insereFaculdadeP
	@nomeFaculdade varchar(70)
	AS
	BEGIN
		IF LEN(@nomefaculdade) <> 0
			INSERT Faculdades VALUES (@nomeFaculdade)
		ELSE
			PRINT 'Entrada inválida.'
	END

	--TESTE--
	EXEC in_Faculdades_insereFaculdadeP 'Ciências Humanas'; 
	-- JÁ EXECUTADO
	

	--> UPDATE DAS MENSALIDADES <-- 
	CREATE PROC up_Cursos_alteraValorCursoP
	@cdCurso  smallint = 0,
	@vlCurso numeric (7,2)
	AS 
	BEGIN
		IF LEN(@cdCurso) <> 0 AND @vlCurso > 0
			UPDATE Cursos SET vl_curso = @vlCurso WHERE cd_curso = @cdCurso
		ELSE
			PRINT 'Parametros inválidos'
	END

	-- TESTE --
	EXEC up_Cursos_alteraValorCursoP 5, 2350.00;
	EXEC up_Cursos_alteraValorCursoP 6, 1274.00;
	EXEC up_Cursos_alteraValorCursoP 7, 5623.69;
	-- JÁ EXECUTADO

	--> SELECT CURSO <--
	ALTER PROC sl_Curso_buscaNomeCursoP
	@nomeCurso varchar(70)
	AS
	BEGIN
		IF LEN(@nomeCurso) <> 0
			SELECT nm_curso FROM Cursos WHERE nm_curso LIKE '%' + @nomeCurso + '%'
		ELSE 
			PRINT 'Parametro Inválido'
	END

	-- TESTE --
	EXEC sl_Curso_buscaNomeCursoP 'pia';
	-- JÁ EXECUTADO

	--> DELETE CURSO <--
	ALTER PROC dl_Curso_deletarCursoP
	@cdCurso smallint = 0
	--Aqui iria o WITH ENCRYPTION para criptografar
	WITH ENCRYPTION
	AS
	BEGIN
		IF LEN(@cdCurso) <> 0 
			DELETE FROM Cursos WHERE cd_curso = @cdCurso
		ELSE
			PRINT 'Deu Ruim'
	END
	-- TESTE --
	EXEC dl_Curso_deletarCursoP 1;
	-- JÁ EXECUTADO

-->** TRIGGER **<--

	CREATE TRIGGER  Faculdades_del_trg ON Faculdades
	FOR DELETE
	AS
	BEGIN
		PRINT 'Não é possível excluir registros da tabela Faculdades.'
		ROLLBACK TRANSACTION -- VOLTA A TRANSAÇÃO ANTERIOR
	END

	-- TESTE --
	DELETE FROM Faculdades WHERE cd_faculdade = 4; -- erro pelo trigger
	DELETE FROM Faculdades; -- erro por dependencia entre tabelas

	-- DESABILITAR TEMPORARIAMENTE --
	DISABLE TRIGGER Faculdades_del_trg ON Faculdades;
	-- HABILITAR TRIGGER -- 
	ENABLE TRIGGER Faculdades_del_trg ON Faculdades;
	-- ALTERAR TRIGGER --
	ALTER TRIGGER  Faculdades_del_trg ON Faculdades
	FOR DELETE
	AS
	BEGIN
		PRINT 'Não é possível excluir registros da tabela Faculdades.'
		ROLLBACK TRANSACTION -- VOLTA A TRANSAÇÃO ANTERIOR
	END
	-- DELETAR TRIGGER --
	DROP TRIGGER Faculdades_del_trg; -- CUIDADO!
	*/

-- TRIGGER PARA IMPEDIR EXCLUSÃO NA TABELA (ROLLBACK)
CREATE TRIGGER Cursos_dl_trg ON Cursos
FOR DELETE
AS
BEGIN
	SELECT * FROM deleted
	ROLLBACK TRANSACTION
	PRINT ('Exclusão não autorizada!')
END

-- TESTE --
	DELETE FROM Cursos WHERE cd_curso = 5;



CREATE TRIGGER Faculdades_in_trg ON Faculdades
FOR INSERT
AS
BEGIN
	SELECT * FROM inserted;
END
-- TESTE --
INSERT INTO Faculdades VALUES ('Direito');



CREATE TRIGGER Faculdades_up_trg ON Faculdades
FOR UPDATE
AS
BEGIN
	SELECT * FROM deleted;
	SELECT * FROM inserted;
END

--TESTE--
UPDATE Faculdades SET nm_faculdade = 'Direiro' WHERE cd_faculdade = 5; 
	


DISABLE TRIGGER Cursos_dl_trg ON Cursos;
DROP TRIGGER Cursos_dl_trg; -- melhor!



CREATE TRIGGER Cursos_dlEng_trg ON Cursos
FOR DELETE
AS 
BEGIN
	IF EXISTS (SELECT * FROM deleted WHERE nm_curso LIKE '%engenharia%')
		BEGIN
			ROLLBACK TRANSACTION
			PRINT ('Não pode excluír cabaço')
		END
	ELSE
		PRINT ('Registro excluído com sucesso')
		
END
select * from Cursos
--TESTE--
DELETE FROM Cursos WHERE cd_curso = 6; -- vai apagar
DELETE FROM Cursos WHERE cd_curso = 5; --não é pra dar certo




ATIVIDADE

CRIAR UM TRIGGER QUE APAGUE UM FUNCIONÁRIO E COPIE PARA A TABELA TEMPORARIA.
PRECISA APAGAR O CODIGO (ESPECIFICAÇÃO DE INDENTIDADE)

OS VALORES QUE ESTÃO PRESTES A SEREM DELETADOS SALVAR EM VARIAVEIS TEMPORÁRIAS
 (SELECT @VARIALVEL = ATRIBUTO)

CRIAR COOLUNA COM DATA E HORA

--DECLARAÇÃO DE VARIAVEL--
DECLARE VARIAVEL TIPO

-->** FIM DA AULA **<--



USE Universidade;

SELECT * FROM Cursos;
SELECT * FROM Faculdades;

CREATE TRIGGER Cursos_dl_trg ON Cursos						-- O nome ficou Cursos_excluirCursos_trg
FOR DELETE
AS
BEGIN
	ROLLBACK TRANSACTION									-- O ideal é primeiro o ROLLBACK depois o print
	PRINT('Exclusão não autorizada')
END;

DELETE FROM Cursos WHERE cd_curso = 3;

ALTER TRIGGER Cursos_excluirCursos_trg ON Cursos
FOR DELETE
AS
BEGIN
	SELECT * FROM deleted									-- A table deleted é temporária
END;

DELETE FROM Cursos WHERE cd_curso = 3;

SELECT * FROM deleted

DISABLE TRIGGER Cursos_excluirCursos_trg ON Cursos;

CREATE TRIGGER Faculdades_in_trg ON Faculdades
FOR INSERT
AS
	SELECT * FROM INSERTED								   -- A tabela inserted é uma tabela temporária dos valores inseridos recentemente

SELECT * FROM Faculdades;

INSERT INTO Faculdades VALUES('Direito');

ALTER TRIGGER Faculdades_up_trg ON Faculdades
FOR UPDATE
AS
BEGIN
	SELECT * FROM deleted;
	SELECT * FROM inserted;								  -- Neste caso deve-se colocar o deleted e o inserted pois o UPDATE nada mais é do que uma inserção depois de uma deleção
END

SELECT * FROM Faculdades;

UPDATE Faculdades SET nm_faculdade = 'Justiça' WHERE cd_faculdade = 4;

ALTER TRIGGER Cursos_dl_trg ON Cursos
FOR DELETE
AS
BEGIN
	IF EXISTS(SELECT * FROM DELETED WHERE nm_curso LIKE 'Engenharia%')						--o IF EXISTS é literalmente se existir
		BEGIN
			ROLLBACK TRANSACTION
			PRINT('Exclusão não autorizada, pois o curso é de engenharia')
		END
	ELSE
			PRINT('Excluído com sucesso')
END;

DELETE FROM Cursos WHERE cd_curso = 11;

SELECT * FROM Cursos;

--INSERT INTO Cursos VALUES('Engenharia Mecânica',1);

||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||PARTE II DA AULA:||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

USE TP3;

SELECT * FROM Funcionarios;

SELECT vl_salario_hora_funcionario AS 'Salario atual',
	   vl_salario_hora_funcionario*1.10 AS 'Salario projetado'
FROM   Funcionarios
WHERE  cd_funcionario>=10 AND cd_funcionario<=30;

UPDATE Funcionarios SET vl_salario_hora_funcionario = 0 WHERE cd_funcionario = 24;

-- OU

UPDATE Funcionarios
SET vl_salario_hora_funcionario = 0
WHERE cd_funcionario = 24;

SELECT * INTO Funcionarios_TMP FROM Funcionarios;							-- Cria uma cópia temporária da tabela desejada
SELECT * FROM Funcionarios_TMP;

DROP TABLE Funcionarios_TMP;

SELECT * INTO Funcionarios_TMP FROM Funcionarios
WHERE cd_funcionario>=10 AND cd_funcionario<=40;

SELECT * FROM Funcionarios_TMP;

BEGIN TRANSACTION;														-- Abre uma transação
SELECT @@TRANCOUNT;

SELECT * FROM Funcionarios_TMP;

UPDATE Funcionarios_TMP SET vl_salario_hora_funcionario = 0;

ROLLBACK TRANSACTION;				-- O ROLLBACK TRANSACTION neste caso quando executado da um 'Ctrl+Z' no código, ele volta com os valores padrão que tinham sido alterados pelo UPDATE

SELECT * FROM Funcionarios_TMP;

SELECT @@TRANCOUNT;					-- A contagem de transações foi zerada após o uso do ROLLBACK TRANSACTION

BEGIN TRAN;
SELECT @@TRANCOUNT;

UPDATE Funcionarios_TMP SET vl_salario_hora_funcionario = vl_salario_hora_funcionario*1.10;

SELECT * FROM Funcionarios_TMP;

COMMIT TRANSACTION;					-- O COMMIT TRANSACTION marca o término de uma transação, não sendo usar o ROLLBACK TRANSACTION neste caso. Zera a contagem de transações também

SELECT * FROM Funcionarios_TMP;

SELECT @@TRANCOUNT;

DROP TABLE Funcionarios_TMP;

SELECT * INTO Funcionarios_TMP		-- Cria uma cópia da tabela Funcionarios mas sem dados
FROM Funcionarios
WHERE 1=2;							-- O 1=2 significa false, porque 1 não é igual a 2

SELECT * FROM Funcionarios_TMP;